...
{
	CE = 0;
	SI24R1_Write_Buf(WRITE_REG + RX_ADDR_P0, ADDRESS1, ADDR_WIDTH);	// 接收设备接收通道0使用和发送设备相同的发送地址
...
{
	CE = 0;
	SI24R1_Write_Buf(WRITE_REG + TX_ADDR, ADDRESS1, ADDR_WIDTH);     // 写入发送地址
	SI24R1_Write_Buf(WRITE_REG + RX_ADDR_P1, ADDRESS1, ADDR_WIDTH);  // 为了应答接收设备，接收通道0地址和发送地址相同
...
//	SI24R1_Write_Reg(WRITE_REG + SETUP_AW, 0x03);			//5个字节的地址宽度
	...
	SI24R1_Write_Buf(WRITE_REG + TX_ADDR, ADDRESS1, ADDR_WIDTH);
	SI24R1_Write_Buf(WRITE_REG + RX_ADDR_P0, ADDRESS1, ADDR_WIDTH);
