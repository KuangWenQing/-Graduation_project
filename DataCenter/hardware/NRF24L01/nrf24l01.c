...
{
	CE = 0;
	SI24R1_Write_Reg(WRITE_REG + RX_PW_P0, PLOAD_WIDTH); //设置接收通道0有效数据宽度
	SI24R1_Write_Buf(WRITE_REG + RX_ADDR_P0, ADDRESS0, ADDR_WIDTH);	// 接收设备接收通道0用和发送设备（通道x）相同的发送地址
...
void SI24R1_RX_Mode_multichannel(void)
{
	CE = 0;
	//我们只有用了3个通道
	SI24R1_Write_Buf(WRITE_REG + RX_ADDR_P0, ADDRESS0, ADDR_WIDTH);	//接收通道0使用和发送设备0相同的发送地址
	SI24R1_Write_Buf(WRITE_REG + RX_ADDR_P1, ADDRESS1, ADDR_WIDTH);	//接收通道1使用和发送设备1相同的发送地址
	SI24R1_Write_Buf(WRITE_REG + RX_ADDR_P2, ADDRESS2, 1);	//接收通道2使用和发送设备2相同的发送地址
	
	SI24R1_Write_Reg(WRITE_REG + RX_PW_P0, PLOAD_WIDTH);			//负载数据长度6个字节
	SI24R1_Write_Reg(WRITE_REG + RX_PW_P1, PLOAD_WIDTH);
	SI24R1_Write_Reg(WRITE_REG + RX_PW_P2, PLOAD_WIDTH);
/*	
...
{
	CE = 0;
	SI24R1_Write_Buf(WRITE_REG + TX_ADDR, ADDRESS0, ADDR_WIDTH);     // 写入发送地址
	SI24R1_Write_Buf(WRITE_REG + RX_ADDR_P0, ADDRESS0, ADDR_WIDTH);  // 为了应答接收设备，接收通道0地址和发送地址相同
...
void SI24R1_TX_Mode_multichannel(void)
{
	...
	SI24R1_Write_Buf(WRITE_REG + TX_ADDR, ADDRESS0, ADDR_WIDTH);
	SI24R1_Write_Buf(WRITE_REG + RX_ADDR_P0, ADDRESS0, ADDR_WIDTH);
...
/*u8 SI24R1_TxPacket(u8 *txbuf)
{
  ...
  SI24R1_Write_Buf(WR_TX_PLOAD, txbuf, PLOAD_WIDTH);	//写数据到TX FIFO,6个字节
 	CE=1;												//CE置高，使能发送	   
